...
bool parser_vm::advance(){//入力コマンドが存在するかどうかを返し、instを変更
    if(getline(in,inst)){
        ...
        else if(inst.substr(0,5)=="label"){commandtype=3;arg1=inst.substr(6,inst.size()-6);int j=0;while(arg1[arg1.size()-1-j]==' '||(int)arg1[arg1.size()-1-j]<32){j++;}arg1=arg1.substr(0,arg1.size()-j-1);}
        else if(inst.substr(0,6)=="return")commandtype=7;
        else if(inst.substr(0,7)=="if-goto"){commandtype=5;arg1=inst.substr(8,inst.size()-8);int j=0;while(arg1[arg1.size()-1-j]==' '||(int)arg1[arg1.size()-1-j]<32){j++;}arg1=arg1.substr(0,arg1.size()-j-1);}
...
int main(){
    std::filesystem::path input1="./FunctionCalls/FibonacciElement/FibonacciElement.vm";
    ...
    else if(v.commandtype==4){//goto
        out<<"@"+v.funcname+"$"+v.arg1+"\n0;JEQ\n";
    }
    else if(v.commandtype==5){//if
        out<<"@SP\nAM=M-1\nD=M\n@"+v.arg1+"\nD;JNE\n";
    }
    else if(v.commandtype==3){//label
        out<<"("+v.funcname+"$"+v.arg1+")\n";
    }
    else if(v.commandtype==6){//func
        out<<"("+v.arg1+")\n";
        rep(i,stoi(v.arg2)){
            out<<"@0\nD=A\n@SP\nM=M+1\nA=M-1\nM=D\n";
        }
    }
    else if(v.commandtype==8){//call
        v.funcname=v.arg1;
        out<<"@"+to_string(num2)+"\nD=A\n@SP\nM=M+1\nA=M-1\nM=D\n";
        out<<"@LCL\nD=M\n@SP\nM=M+1\nA=M-1\nM=D\n";
        out<<"@ARG\nA=M\nD=M\n@SP\nM=M+1\nA=M-1\nM=D\n";
    ...
    else if(v.commandtype==7){//return
        out<<"@LCL\nD=M\n@R13\nM=D\n";
        out<<"@5\nA=D-A\nD=M\n@R14\nM=D\n";//戻り値アドレス
        out<<"@SP\nM=M-1\nA=M\nD=M\n@ARG\nA=M\nM=D\n";
        out<<"@ARG\nD=M\n@SP\nM=D+1\n";
